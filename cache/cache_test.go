...
func TestRedisCache_Forget(t *testing.T) {
	...
	if inCache {
		t.Error("alpha found in cache when it should be forgotten")
	}
}
...
func TestRedisCache_EmptyByMatch(t *testing.T) {
	...
	if inCache {
		t.Error("alpha2 found in cache when it should be forgotten")
	}
	...
	if !inCache {
		t.Error("beta not found in cache when it should exist")
	}
}
